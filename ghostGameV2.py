...
def playGame():
    ...
    playerPos, ghosts, gameBoard, discBoard = houseGen.generateGameBoard(
        SCREEN_WIDTH, SCREEN_HEIGHT - HUD_HEIGHT, MAX_ROOMS, ROOM_MIN_SIZE,
        ROOM_MAX_SIZE)
    ...
    while not (dead or done):
        ...
        if dead:
            print("you have died of ghost! you survived for " + str(time) +
                  " turns.")
            return askToPlayAgain(getch)
        elif done:
            graphics.renderAll(playerPos, ghosts, gameBoard, discBoard)

            print("you win! you did it in  " + str(time) +
                  " turns. there were " + str(len(ghosts)) + " ghosts")
...
def checkDead(playerPos, ghosts):
    ...
    for ghost in ghosts:
        #print(ghost)
        if (playerPos[0] == ghost[0]
                and playerPos[1] == ghost[1]) and not DEBUG:
            return True
    return False
